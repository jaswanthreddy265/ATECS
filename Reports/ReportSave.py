...
class ReportExcel():
    def __init__(self):
        pass
    def SaveProcess(self, savetestname='All', systemname='', fromdate='', todate=''):
        if savetestname == 'All':
            GetFreqIndexTable = FreqAccIndxTableDB()
            GetFreqIndexTable.SelFreqDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            GetPwIndexTable.SelPwDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            GetPriIndexTable.SelPriDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            GetAmplIndexTable.SelAmplDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            GetDoaIndexTable.SelDoaDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            GetSensMeasIndexTable.SelSensMeasDateFilter(datefilterfrom=fromdate, datefilterto=todate)
            ...
            dataappend = pd.concat( [freqdataappend, pwdataappend, pridataappend, ampldataappend, doadataappend, sensmeasdataappend],
                ignore_index=True)
        elif savetestname == 'Frequency Test':
            GetFreqIndexTable = FreqAccIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetFreqIndexTable.SelFreqDateSysFilter(datefilterfrom=fromdate, datefilterto=todate, sysfilter=systemname.lower())
                dataappend = GetFreqIndexTable.FreqDateSysFil
            else:
                GetFreqIndexTable.SelFreqDateFilter(datefilterfrom=fromdate, datefilterto=todate)
                dataappend = GetFreqIndexTable.FreqDateFil

        elif savetestname == 'PulseWidth Test':
            GetPwIndexTable = PwMeasIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetPwIndexTable.SelPwDateSysFilter(datefilterfrom=fromdate, datefilterto=todate,sysfilter=systemname.lower())
                dataappend = GetPwIndexTable.PwDateSysFil
            else:
                GetPwIndexTable.SelPwDateFilter(datefilterfrom=fromdate, datefilterto=todate)
                dataappend = GetPwIndexTable.PwDateFil
        elif savetestname == 'PRI Test':
            GetPriIndexTable = PriMeasIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetPriIndexTable.SelPriDateSysFilter(datefilterfrom=fromdate, datefilterto=todate,sysfilter=systemname.lower())
                dataappend = GetPriIndexTable.PriDateSysFil
            else:
                GetPriIndexTable.SelPriDateFilter(datefilterfrom=fromdate, datefilterto=todate)
                dataappend = GetPriIndexTable.PriDateFil
        elif savetestname == 'DOA Test':
            GetDoaIndexTable = DoaMeasIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetDoaIndexTable.SelDoaDateSysFilter(datefilterfrom=fromdate, datefilterto=todate,sysfilter=systemname.lower())
                dataappend = GetDoaIndexTable.DoaDateSysFil
            else:
                GetDoaIndexTable.SelDoaDateFilter(datefilterfrom=fromdate, datefilterto=todate)
                dataappend = GetDoaIndexTable.DoaDateFil
        elif savetestname == 'Amplitude Test':
            GetAmplIndexTable = AmplAccIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetAmplIndexTable.SelAmplDateSysFilter(datefilterfrom=fromdate, datefilterto=todate,sysfilter=systemname.lower())
        ...
        elif savetestname == 'Sensitivity Test':
            GetSensIndexTable = SensMeasIndxTableDB()
            if (systemname == 'ESM') or (systemname == 'RFPS') or (systemname == 'RWR') or (systemname == 'WARNER'):
                GetSensIndexTable.SelSensMeasDateSysFilter(datefilterfrom=fromdate,datefilterto=todate,sysfilter=systemname.lower())
                dataappend = GetSensIndexTable.SensMeasDateSysFil
            else:
                GetSensIndexTable.SelSensMeasDateFilter(datefilterfrom=fromdate,datefilterto=todate)
        ...
        for i in range(0, len(TableSelection)):
            ...
            if (self.CB_Test.currentText() == 'All'):
                ...
                workbook = load_workbook(
                    "C:/Users/jaswa/PycharmProjects/ATEC1/Reports/Templates/FreqAccTestTemplate.xlsx")
                ...
                self.FontSizeStyle = [f'B{self.tableposition}', f'B{self.tableposition + 3}',
                                      f'B{self.tableposition + 6}', f'B{self.tableposition + 8}',
                                      f'H{self.tableposition + 8}']
