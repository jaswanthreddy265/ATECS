...
from MainSrc.printtestC import PrinterDlg
...
########################################################################################################################
    ...
    def CalibRfPathLoss(self):
        ...
        self.RFPath_tableWidget.setHorizontalHeaderLabels(
            ["S.No", "Set Frequnecy\n(MHz)", "Set Power\n(dBm)", "Measured Power\n(dBm)", "Path Loss\n(dB)"])
        self.RFPath_tableWidget.horizontalHeader().setStretchLastSection(True)
        self.RFPath_tableWidget.setColumnWidth(0, 35)
        self.RFPath_tableWidget.setColumnWidth(1, 90)
        self.RFPath_tableWidget.setColumnWidth(2, 70)
        self.RFPath_tableWidget.setColumnWidth(3, 110)
        self.RFPath_tableWidget.setColumnWidth(4, 80)
        self.startfreq = float(self.lineEdit_Rad_Start_Freq.text())
        self.stopfreq = float(self.lineEdit_Rad_Stop_Freq.text())
        self.stepfreq = float(self.lineEdit_Rad_Step_Freq.text())
        self.setampl = float(self.lineEdit_Rad_Ampl.text())
        QApplication.processEvents()
        time.sleep(1)
        while self.startfreq <= self.stopfreq:
            self.PerfCalibRfPath()
            self.RFRadProgressBar()
            QApplication.processEvents()
            time.sleep(1)
            self.startfreq = self.startfreq + self.stepfreq
            self.setampl = self.setampl + 2
        self.PB_Rad_Calib.setDisabled(False)
        self.PB_Rad_Abort.setDisabled(True)
    def PerfCalibRfPath(self):
        self.measampl = self.setampl + random.randrange(-1, 1)
        self.pathloss = self.setampl - self.measampl
        self.RFPath_tableWidget.setRowCount(self.row + 1)
        self.RFPath_tableWidget.setItem(self.row, 0, QTableWidgetItem(str(self.RFPath_tableWidget.rowCount())))
        self.RFPath_tableWidget.setItem(self.row, 1, QTableWidgetItem(str(self.startfreq)))
        self.RFPath_tableWidget.setItem(self.row, 2, QTableWidgetItem(str(self.setampl)))
        self.RFPath_tableWidget.setItem(self.row, 3, QTableWidgetItem(str(self.measampl)))
        self.RFPath_tableWidget.setItem(self.row, 4, QTableWidgetItem(str(self.pathloss)))
        tableverticalscroll = self.RFPath_tableWidget.item(self.row, 0)
        self.RFPath_tableWidget.scrollToItem(tableverticalscroll, QAbstractItemView.PositionAtTop)
        self.RFPath_tableWidget.selectRow(self.row)
        self.row = self.row + 1
    def RFRadProgressBar(self):
        No_Of_Steps = ((self.stopfreq - self.startfreq)) / self.stepfreq + 1
        percentage = int(self.row * 100 / No_Of_Steps)
        self.Rad_progressBar.setValue(percentage)
    def CalRfPathPlot(self):
        col_count = 5
        ...
        for row in range(row_count):
            ...
            data.append(rowdata)
        self.df = pd.DataFrame(data, columns=["S.No", "Set Frequnecy\n(MHz)", "Set Power\n(dBm)", "Measured Power\n(dBm)",
                                              "Path Loss\n(dB)"])
        rf_df_freq = self.df['Set Frequnecy\n(MHz)'].tolist()
        self.setfreqvalue = [float(i) for i in rf_df_freq]
        rf_df_loss = self.df['Path Loss\n(dB)'].tolist()
        self.pathlossvalue = [float(i) for i in rf_df_loss]
        self.RFPath_graph.setTitle('<font size="15"><strong>RF Path Loss Calibration</strong></font>', color='blue')
        self.RFPath_graph.setLabel("bottom", '<span style="color: green; font-size: 18px">Path Loss (dBm)</span>')
        self.RFPath_graph.setLabel("left", '<span style="color: green; font-size: 18px">Frequency (MHz)</span>')

        self.PlotGraph.addLegend()
        self.PlotGraph.setYRange(-100, 100)
        self.PlotGraph.plot(self.setfreqvalue, self.pathlossvalue, pen='m', width=2, name='')
    ####################################################################################################################
    def graph_plot(self):
        x1 = [2, 3, 4, 5]
        y1 = [10, 20, 15, 5]
