...
class MainGUI(QMainWindow, Ui_ATEC_App):
    def __init__(self):
        ...
        # CALIBRATION TAB
        self.row = 0
        self.PB_RFPath_Calib.clicked.connect(self.CalibRfPathLoss)
        self.PB_RFPath_Abort.setDisabled(True)
        ...
        #self.PB_Calibration.clicked.connect(lambda:self.stackedWidget_2.setCurrentWidget(self.Calibration_Amplitude))
        self.PB_Calibration.clicked.connect(self.Calib_Config)
        ...
        # Calibration Tab Events (system (Rwr/Warner,ESMP) Calibration(Amplitude,ADF Slope),RFCablePath(Radiation Mode,Injection Mode))
        self.Calib_ESMP.clicked.connect(self.Calib_EsmpConfig)
        self.Calib_RWR.clicked.connect(self.Calib_RwrConfig)
        self.Calib_Amplitude.clicked.connect(self.Calib_AmplConfig)
        self.Calib_ADFSlope.clicked.connect(self.Calib_AdfSlopeConfig)
        self.Calib_Radiation.clicked.connect(self.Calib_RadConfig)
        self.Calib_Injection.clicked.connect(self.Calib_InjectConfig)

        # Calibration Tab Buttons Disabled (Abort,Save to File,Send to Target,Radiation Mode Abort)
        self.PB_Ampl_Abort_Cal.setDisabled(True)
        self.PB_Ampl_SaveFile.setDisabled(True)
        self.PB_Ampl_Send_Target.setDisabled(True)
        self.PB_RFPath_Abort.setDisabled(True)
        self.PB_ADFSlope_SaveFile.setDisabled(True)
        self.PB_ADFSlope_Send_Target.setDisabled(True)

########################################################################################################################
            ###################################Calibration###########################################
    #################Cable Loss (Radiation) #########################
    def Calib_RadConfig(self):
        self.stackedWidget_2.setCurrentWidget(self.Cable_Loss)
        self.frame_RwrCalibration.setVisible(False)
        self.Calib_ESMP.setChecked(False)
        self.Calib_RWR.setChecked(False)
        self.Calib_Injection.setChecked(False)

    #################Cable Loss (Injection) #########################
    def Calib_InjectConfig(self):
        self.stackedWidget_2.setCurrentWidget(self.Cable_Loss)
        self.frame_RwrCalibration.setVisible(False)
        self.Calib_ESMP.setChecked(False)
        self.Calib_RWR.setChecked(False)
    ...
    ##################### System (ESMP) Calibration ################
    def Calib_EsmpConfig(self):
        self.frame_RwrCalibration.setVisible(False)
        self.Calib_RWR.setChecked(False)
        self.Calib_Radiation.setChecked(False)
        self.Calib_Injection.setChecked(False)
        self.stackedWidget_2.setCurrentWidget(self.Calibration_ESMP)

    ##################### RWR/Warner Calibration, Amplitude ##################
    def Calib_AmplConfig(self):
        self.stackedWidget_2.setCurrentWidget(self.Calibration_Amplitude)
        self.Calib_ADFSlope.setChecked(False)

    ##################### RWR/Warner Calibration, ADF Antenna Slope ##############
    def Calib_AdfSlopeConfig(self):
        self.stackedWidget_2.setCurrentWidget(self.Calibration_ADFSlope)
        self.Calib_Amplitude.setChecked(False)

    ##################### System (RWR/WARNER) Calibration ################
    def Calib_RwrConfig(self):
            self.Calib_ESMP.setChecked(False)
            self.Calib_Injection.setChecked(False)
            self.Calib_Radiation.setChecked(False)
            self.frame_RwrCalibration.setVisible(True)
            if self.Calib_Amplitude.isChecked():
                self.Calib_AmplConfig()
            elif self.Calib_ADFSlope.isChecked():
                self.Calib_AdfSlopeConfig()

    #############################Calibration Tab Update ###############################
    def Calib_Config(self):
        if self.Calib_ESMP.isChecked():
            self.Calib_EsmpConfig()
        elif self.Calib_RWR.isChecked():
            self.Calib_RwrConfig()
        elif self.Calib_Radiation.isChecked():
            self.Calib_RadConfig()
        elif self.Calib_Injection.isChecked():
            self.Calib_InjectConfig()

    ################## Calibration Parameters, status frame(hide) ############
...
########################################################################################################################
            ############********* System Tab (Add Button To ListWidget) *******###################
    def Sys_Add(self):
...
#############################************************CALIBRATION*********************###################################
    def CalibRfPathLoss(self):
        self.RFPath_progressBar.setValue(0)
        self.RFPathls_grph.clear()
        self.RFPathls_grph.setTitle('<font size="15"><strong>RF Path Loss Calibration</strong></font>', color='blue')
        self.RFPathls_grph.setLabel("bottom", '<span style="color: green; font-size: 18px">Frequency (MHz)</span>')
        self.RFPathls_grph.setLabel("left", '<span style="color: green; font-size: 18px">Path Loss (dB)</span>')
        self.PB_RFPath_Calib.setDisabled(True)
        self.PB_RFPath_Abort.setDisabled(False)
        ...
        startfreq = float(self.lineEdit_RFPath_Start_Freq.text())
        ...
        while freq < stopfreq:
            ...
            freq = freq + stepfreq
            rowindx = rowindx + 1
        self.PB_RFPath_Calib.setDisabled(False)
        self.PB_RFPath_Abort.setDisabled(True)
    ####################################################################################################################
    ...
    def UpdateRfPathLossPlot(self,set_freq_list = [], path_loss_list = []):
        ...
        if self.Calib_Radiation.isChecked():
            ...
            self.RFPathls_grph.plot(setfreqvalue, pathlossvalue, pen='m', width=2, name='injection')
    ####################################################################################################################
    def RfPathDBUpdate(self):
        ...
        if self.Calib_Radiation.isChecked():
            self.CalRfPathLossClass.RfCalUpdateToDB(calrffreqfrom=self.lineEdit_RFPath_Start_Freq.text(), calrffreqto=self.lineEdit_RFPath_Stop_Freq.text(), updatecaltbname='rfpathlossradmode',calrftabdata = calrftabgetdata)
        elif self.Calib_Injection.isChecked():
            self.CalRfPathLossClass.RfCalUpdateToDB(calrffreqfrom=self.lineEdit_RFPath_Start_Freq.text(), calrffreqto=self.lineEdit_RFPath_Stop_Freq.text(), updatecaltbname='rfpathlossinjmode', calrftabdata=calrftabgetdata)
    ####################################################################################################################
