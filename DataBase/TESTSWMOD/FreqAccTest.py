...
class FreqAccTestMain(QMainWindow, Ui_FreqAccTestP):
    ...
    def ConnectDB(self):
        ...
        self.indxtb = FreqAccIndxTableDB()
        #self.indxtb.tablename = self.ConfigJSON['indxtabname']
        self.datatb = FreqAccTestTableDB()
        """DataBaseSettings = {
            'user': 'postgres',
    ...
    def AppendNewRecordtoIndxTable(self):
        timestamp = datetime.datetime.now()
        table_ref_id = f'''{self.comboBox_System_Freq.currentText().lower()}freqacctest_{timestamp.strftime("%Y_%m_%d_%H_%M_%S")}'''
        print(table_ref_id)
        ...
        if self.CB_SigCat_Freq.currentText() == "CW":
            ...
            newrow = {'date': timestamp, 'username': 'ravi', 'system_id': 1, 'system': self.comboBox_System_Freq , 'mode': self.comboBox_Mode_Freq,
            ...
            self.indxtb.AddFreqRecord(usercred=newrow)
  ...
  ####################################################################################################################
    def AppendtoDataTable(self, newrow={'set_freq':500,'meas_freq':4,'error':5}):
        ...
        self.datatb.AddFreqRecord(freqtestvalues=test_record)
