...
class AnalogGaugeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # LOAD CUSTOM FONT
        ################################################################################################     
        QFontDatabase.addApplicationFont(os.path.join(os.path.dirname(__file__), 'fonts/Arial/Arial.ttf') )
        ...
        self.initial_scale_fontsize = 30
        ...
        self.initial_value_fontsize = 45
    ...
    def setGaugeTheme(self, Theme = 1):
        ...
        if Theme == 2:
            ...
            self.set_scale_polygon_colors([[1, QColor("#ffffcc")]])
    ...
    def create_values_text(self):
        ...
        # angle_distance = (float(self.scale_angle_size) / float(self.scalaCount))
        # for i in range(self.scalaCount + 1):
        self.meas = str(int(self.value))
        w = fm.width(self.meas) + 1
        h = fm.height()
        ...
        y = text_radius * math.sin(math.radians(angle))
        # print(w, h, x, y, text)
        text = [x - int(w/2), y - int(h/2), int(w), int(h), Qt.AlignCenter, self.meas]
        #print("Values:",self.meas)
        painter.drawText(int(text[0]), int(text[1]), int(text[2]), int(text[3]), int(text[4]), text[5])
